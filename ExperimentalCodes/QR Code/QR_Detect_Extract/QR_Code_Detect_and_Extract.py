...
AB=BC=CA= dist=slope=areat=arear=areab=large=padding=0
...
while True:
    ...
    if (mark >= 3):
        ...
        if(top < len(contours) and right < len(contours) and bottom < len(contours) and cv2.contourArea(contours[top]) > 10 and cv2.contourArea(contours[right]) > 10 and cv2.contourArea(contours[bottom]) > 10):
            ...
            warped = transform.four_point_transform(out,src)

            cv2.imshow("warped",warped)
            cv2.circle(out,N,1,(0,0,255),2)
            cv2.drawContours(out,contours,top,(255,0,0),2)
            cv2.drawContours(out,contours,right,(0,255,0),2)
            cv2.drawContours(out,contours,bottom,(0,0,255),2)
